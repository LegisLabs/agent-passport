"""Append-only, hash-chained audit log with authority-signed receipts.
...
a JWT signed by the authority over the decision and the chain hash, so the
agent, the bank and the authority all hold proof of what was decided and why.
"""
...
def record(kind: str, subject: str | None, entry: dict, receipt_for: dict | None = None) -> dict:
    ...
    if receipt_for is not None:
        receipt = crypto.sign_jwt("authority", {
            "typ": "agent-passport-receipt",
            "iss": crypto.signer("authority")["kid"],
            "iat": crypto.now_ts(),
            "audit_hash": h,
            "prev_hash": prev,
            **receipt_for,
