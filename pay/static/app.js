...
const AP = (() => {
  ...
  async function customer() {
    ...
    function renderEvent() {
      const r = acRows.find(x => x.id === eventId); const box = $('cu-event-body');
      if (!r) { box.innerHTML = '<p class="error">No record with that number on your account.</p>'; return; }
      const e = r.entry || {}, i = e.instruction || {}; const n = noteFor(r) || { tone: 'slate', label: r.kind, text: plainEvent(r), sub: '' };
      const ap = approverOf(r.subject), sup = esc(i.supplier_name || 'a payee'), amt = gbp(i.amount), ag = esc(agentOf(r.subject));
      ...
      // three plain lines: what was tried, what the bank did, what it means for you
      let tried = i.amount != null ? `${ag} asked to pay <b>${sup}</b> <b>${amt}</b>${i.invoice_ref ? ` for invoice ${esc(i.invoice_ref)}` : ''}.` : '';
      let did = '', means = '';
      if (r.kind === 'verify' && e.decision === 'DENY') {
        const why = e.code === 'PAYEE_NOT_ON_MANDATE' ? `The invoice asked for a different bank account from the one you approved for ${sup}.` : e.code === 'REPLAY_DETECTED' ? 'The same payment was sent twice; the bank paid it once.' : e.code === 'AGENT_SIGNATURE_INVALID' ? 'The request did not come from your agent\'s own key.' : e.code === 'PER_PAYMENT_LIMIT_EXCEEDED' ? `That is more than the ${gbp((ad.per_payment_limit || {}).amount)} a payment you allow.` : e.code === 'MONTHLY_LIMIT_EXCEEDED' ? `That would take ${sup} past the ${gbp((ad.monthly_limit_per_account || {}).amount)} you allow in 30 days.` : e.code === 'DAILY_COUNT_EXCEEDED' ? 'Your agent had already made its daily number of payments.' : e.code === 'CURRENCY_NOT_PERMITTED' ? 'It was not in pounds; your mandate is in pounds only.' : e.code === 'OUT_OF_SCOPE' ? 'That is not something your mandate lets it do.' : ['MANDATE_EXPIRED', 'MANDATE_REVOKED', 'MANDATE_NOT_SIGNED'].includes(e.code) ? 'Your mandate is no longer in force.' : 'It failed one of the bank\'s checks.';
        did = `<b>The bank refused it.</b> ${why}`;
        means = classId(e.failure_class) === 'fraud' ? 'Nothing left your account. Nothing for you to do; the bank\'s risk team has the record.' : 'Nothing left your account. If this keeps happening, it is worth raising with your provider.';
      } else if (r.kind === 'verify' && e.decision === 'ESCALATE') {
        const dd = decisionFor(r.id);
        did = e.code === FIRST ? '<b>The bank is holding it</b> because it is the first payment under your new mandate.' : `<b>The bank is holding it</b> because it is above the ${gbp(hold)} you asked the bank to check with ${esc(ap.name || 'your approver')}.`;
        means = dd ? `${esc(((dd.entry.decided_by || dd.entry.approver) || {}).name || 'Your approver')} ${dd.entry.outcome === 'RELEASED' ? 'released it and it has been paid' : 'refused it and nothing left your account'}.` : e.code === FIRST ? 'Nothing moves until you confirm or refuse it below.' : `Nothing moves until ${esc(ap.name || 'your approver')} decides.`;
      } else if (r.kind === 'decision') {
        const who = esc(((e.decided_by || e.approver) || {}).name || 'Your approver'); const rel = e.outcome === 'RELEASED';
        did = `<b>${who} ${rel ? (e.first_payment ? 'confirmed' : 'released') : 'refused'} it.</b>`;
        means = rel ? 'It has been paid.' : 'Nothing left your account.';
      } else { did = n.text; }
      const decidedFirst = n.first && !decisionFor(r.id);
      const details = `<details class="cu-event__details"><summary>Details for the record</summary>${r.kind === 'verify' ? `<h4 class="h4">The bank's checks</h4><ol class="bd-trace">${(e.trace || []).map(st => `<li class="${st.ok ? 'ok' : (e.decision === 'ESCALATE' && st.rule === e.rule ? 'hold' : 'fail')}"><b>${esc(st.rule)}</b> ${esc(st.title)}<span>${esc(st.note)}</span></li>`).join('')}</ol>` : ''}<h4 class="h4">Proof</h4><dl class="kv kv--card"><div><dt>Record</dt><dd>#${r.id} <span class="mono small">${esc(r.hash)}</span></dd></div><div><dt>Receipt</dt><dd>${r.receipt ? 'signed by the bank' : 'none'}</dd></div><div><dt>Time</dt><dd>${d(r.ts)} ${t(r.ts)}</dd></div></dl><div class="actions">${(state.violations || []).find(v => v.audit_id === r.id) ? `<a class="btn btn--secondary btn--small" href="/api/evidence/violations/${(state.violations || []).find(v => v.audit_id === r.id).id}" target="_blank" rel="noopener">Export the evidence</a>` : ''}${r.kind === 'verify' ? `<button class="btn btn--secondary btn--small" type="button" id="cu-event-replay">Replay the decision</button><span class="actions__note" id="cu-event-replay-note"></span>` : ''}</div></details>`;
      box.innerHTML = `<div class="cu-event__hero cu-event__hero--${n.tone}"><span class="tag tag--${n.tone === 'red' ? 'red' : n.tone === 'amber' ? 'amber' : 'grey'}">${esc(n.label)}</span><h2 class="cu-event__title">${n.text}</h2><p class="small">${d(r.ts)} at ${t(r.ts)}</p></div><ol class="cu-event__steps">${tried ? `<li><span>What happened</span><p>${tried}</p></li>` : ''}<li><span>What the bank did</span><p>${did}</p></li><li><span>What it means for you</span><p>${means}</p></li></ol>${decidedFirst ? `<div class="cu-event__act"><button class="btn" type="button" data-review="${r.id}">${pendingConfirm === r.id ? 'Hide the review' : 'Review and confirm'}</button></div>${pendingConfirm === r.id ? confirmHtml(n) : ''}` : ''}${details}`;
