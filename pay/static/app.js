...
const AP = (() => {
  ...
  async function customer() {
    ...
    async function refreshDecisions() { if (!snap) return; try { const live = await api('GET', '/api/state'); const ids = new Set((live.decisions || []).map(x => x.entry.held_audit_id)); lastDecisions = [...(live.decisions || []), ...snap.state.decisions.filter(x => !ids.has(x.entry.held_audit_id))]; snap.state.decisions = lastDecisions; snap.audit.rows = [...(live.decisions || []).filter(x => !snap.audit.rows.some(y => y.id === x.id)), ...snap.audit.rows]; } catch (e) { /* live unavailable: the snapshot stands */ } }
