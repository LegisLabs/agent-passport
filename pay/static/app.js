...
const AP = (() => {
  ...
  // every instruction resolves to one status: Processed, or For review (with its class when a check failed); a decided hold shows what the person decided. Nothing is refused outright
  let lastDecisions = [];
  ...
  const statusChip = (r) => {
    ...
    if (e.decision === 'ESCALATE') {
      ...
      const failed = !!e.failed_check, id = classId(e.failure_class);
      if (!dd) return first ? chip('amber', 'Held · first payment', 'the first payment under this mandate version waits for the customer to review and confirm it once') : failed ? chip(id === 'fraud' ? 'red' : 'amber', 'For review · ' + CLASSES[id][0].toLowerCase(), CLASS_LEGEND[id] + ' Not executed until a person approves it with a note.') : chip('amber', 'For review', 'inside the mandate, above the bank\'s hold condition; a named person approves or declines it with a note');
      if (dd.entry.outcome === 'RELEASED') return chip('green', first ? 'Processed · confirmed by the customer' : 'Processed · approved after review', first ? 'the customer reviewed and confirmed its first payment under this mandate; recorded in the chain' : 'held, then approved by a named person with a note; recorded in the chain');
      return chip('grey', first ? 'Declined · by the customer' : 'Declined after review', first ? 'the customer declined the first payment under this mandate; recorded in the chain' : 'held, then declined by a named person with a note; recorded in the chain');
    }
    return chip('grey', esc(e.decision || ''), '');
  };
  ...
  async function bank() {
    ...
    async function renderDash() {
      ...
      attention.slice(0, 5).forEach(r => {
        ...
        const action = !held ? `<span class="small">${esc(e.rule)} ${esc((e.code || '').toLowerCase().replace(/_/g, ' '))}</span> · <a href="/api/evidence/violations/${esc(String((vById[r.id] || {}).id || ''))}" target="_blank" rel="noopener">evidence</a>`
          ...
          : `<button class="btn btn--small" type="button" data-decide="release" data-audit="${r.id}">Approve</button> <button class="btn btn--small btn--warning" type="button" data-decide="refuse" data-audit="${r.id}">Decline</button>`;
        const tr = el('tr', 'bd-log__row bd-att__row' + (held && e.failed_check && classId(e.failure_class) === 'fraud' ? ' bd-att__row--fraud' : ' bd-att__row--held'), `<td class="mono small">${t(r.ts)}</td><td>${esc(agentName(r.subject))}<br><span class="mono small">${esc(r.subject || '')}</span></td><td>${esc(i.supplier_name || '')} <span class="mono small">${esc(i.payee_account_ref || '')}</span></td><td class="num mono">${gbp(i.amount)}</td><td>${statusCell(r)}</td><td class="bd-att__act">${action}</td>`);
      ...
      $('bd-attention-meta').textContent = attention.length ? `${heldOpen.length} for review · ${fraudRows.length} fraud indicator${fraudRows.length === 1 ? '' : 's'}` : '';
      ...
      viol.filter(x => !x.synthetic).slice(0, 5).forEach(x => ev.append(el('li', null, `<span>Flag #${x.id}, ${esc(x.rule)} at ${t(x.ts)}</span><a href="/api/evidence/violations/${x.id}" target="_blank" rel="noopener">Export</a>`)));
    ...
    function renderStats(verifies, viol, live) {
      ...
      $('bd-stats-totals').innerHTML = [[num(verifies.length), 'Instructions'], [num(pays.length), 'Processed'], [gbp(value), 'Value processed'], [num(held), 'Held'], [num(nFraud), 'Flagged, fraud indicator'], [num(nErr), 'Flagged, agent error']].map(([n, l]) => `<li><b class="mono">${n}</b><span>${l}</span></li>`).join('');
    ...
    function renderGraphs(verifies, viol, live) {
      ...
      const bp = o.by_product || {}; bars('chart-product', Object.keys(bp).map(k => k.replace(/ \(.*\)$/, '')), Object.values(bp), (v_) => gbp(v_));
      ...
      const ol = ['Processed', 'Held for a person', 'Flagged, fraud indicator', 'Flagged, agent error'], ov = [proc, held, nf, ne], oc = ['#1f8a4c', '#b45f06', '#c62828', '#97a39f'];
    ...
    function heldHtml(r) {
      ...
      const confirmBox = armed ? `<div class="bd-confirm" role="group" aria-label="Confirm the decision"><p><strong>${armed === 'release' ? 'Release' : 'Refuse'} ${gbp(i.amount)} to ${esc(i.supplier_name || '')}</strong> <span class="mono small">${esc(i.payee_account_ref || '')}</span>${i.invoice_ref ? ` · invoice <span class="mono">${esc(i.invoice_ref)}</span>` : ''}.<br>Decided for ${esc(ap.name || '')}, ${esc(ap.role || '')}, the customer's named approver. Recorded as <strong>${esc(who())}</strong> at ${t(new Date().toISOString())}, signed by the bank into the evidence chain${armed === 'release' ? '; the payment executes' : '; nothing moves'}.</p><label class="label" for="bd-note-${r.id}">Your note, recorded with the decision</label><textarea class="textarea" id="bd-note-${r.id}" rows="2" placeholder="${armed === 'release' ? 'e.g. Confirmed the order with the supplier by phone; approved this once.' : 'e.g. Supplier confirms no such invoice; declined.'}"></textarea><p class="error" id="bd-note-err-${r.id}" hidden>Write a note first; it is recorded with your decision.</p><div class="actions"><button class="btn btn--small${armed === 'refuse' ? ' btn--warning' : ''}" type="button" data-confirm="${armed}" data-audit="${r.id}">${armed === 'release' ? 'Approve and pay' : 'Decline and record'}</button><button class="btn btn--secondary btn--small" type="button" data-cancel-decide="${r.id}">Cancel</button></div></div>` : '';
      const why = firstPay ? `${gbp(i.amount)} is inside the mandate and the hold condition; it is the first payment under mandate version ${esc(String((x.mandate || {}).version || 1))}, so the verifier returned ESCALATE at R.9 for the customer's own confirmation` : e.failed_check ? `check ${esc(e.rule)} failed (${esc((e.code || '').toLowerCase().replace(/_/g, ' '))}): ${esc(e.reason)}. Nothing is refused outright; a person approves or declines it with a note` : `${gbp(i.amount)} is inside the mandate (up to ${gbp((ad.per_payment_limit || {}).amount)} a payment) and above the bank's hold condition of ${gbp(hold)}; the verifier returned ESCALATE at R.9`;
      ...
      const decision = dd ? `<b>${dd.entry.outcome === 'RELEASED' ? (firstPay ? 'Confirmed' : 'Approved') : 'Declined'}</b> at ${t(dd.ts)} by ${esc(dd.entry.officer)} · ${esc(dd.entry.reason)} · chain entry <span class="mono small">#${dd.id} ${esc(dd.hash.slice(0, 10))}</span>${dd.receipt ? ' · receipt signed by the bank' : ''}`
        : firstPay ? `<span class="small">awaiting the customer · <a href="/customer?ref=${esc(r.subject)}">open the customer dashboard</a></span>`
        : armed ? '<span class="small">confirm below</span>' : `<span class="bd-hold__act"><button class="btn btn--small" type="button" data-decide="release" data-audit="${r.id}">Approve</button> <button class="btn btn--small btn--warning" type="button" data-decide="refuse" data-audit="${r.id}">Decline</button> <span class="small">either way the chain records who decided, when, why, and on what</span></span>`;
      return `<div class="bd-hold"><h3 class="h4">${firstPay ? 'Held for the customer: first payment under the mandate' : 'Held for review'}</h3>${kv([['Why', why], ['Mandate', `${esc(ap.name || '')} signed version ${esc(String((x.mandate || {}).version || 1))} · up to ${gbp((ad.per_payment_limit || {}).amount)} a payment · ${gbp((ad.monthly_limit_per_account || {}).amount)} per account in 30 days · ${(ad.supplier_allowlist || []).length} payee accounts`], ['Who decides', whoDecides], ['Decision', decision]])}${confirmBox}</div>`;
    }
    async function decideHeld(auditId, decision) {
      let out = null;
      const note = ($(`bd-note-${auditId}`) || { value: '' }).value.trim();
      if (!note) { const er = $(`bd-note-err-${auditId}`); if (er) er.hidden = false; const b = document.querySelector(`[data-confirm][data-audit="${auditId}"]`); if (b) b.disabled = false; return; }
      try { out = await api('POST', `/api/audit/${auditId}/decide`, { decision, reason: note, by: 'bank' }); } catch (e) { alert(e.message); }
      pendingDecide = null; expandedAtt = null;
      if (out) { const done = $('bd-attention-done'); done.hidden = false; done.innerHTML = `<b>${out.outcome === 'RELEASED' ? 'Approved' : 'Declined'}</b> by ${esc(out.officer)} for ${esc((out.approver || {}).name || '')} · chain entry <span class="mono">#${out.audit_id} ${esc(out.hash.slice(0, 10))}</span>, receipt signed by the bank${out.settlement ? ` · ${esc(out.settlement.rail_reason || 'executed')}` : ' · nothing moved'}. Now in recent activity, the statistics and the evidence trail.`; setTimeout(() => { done.hidden = true; }, 12000); }
      await refresh();
    }
    ...
    function detailHtml(r, vio) {
      ...
      const trace = (e.trace || []).map(s => `<li class="${s.ok ? 'ok' : (e.failed_check ? 'fail' : 'hold')}"><b>${esc(s.rule)}</b> ${esc(s.title)}<span>${esc(s.note)}</span></li>`).join('');
      const kv = (pairs) => `<dl class="kv">${pairs.map(([k, val]) => `<div><dt>${k}</dt><dd>${val}</dd></div>`).join('')}</dl>`;
      return `<div class="bd-detail__grid">
        ...
        <div><h3 class="h4">The nine checks, in order</h3><ol class="bd-trace">${trace}</ol><p class="bd-verdict bd-verdict--${e.decision}${e.failure_class === 'agent_error' ? ' bd-verdict--quiet' : ''}"><b>${esc(e.decision)}</b> ${esc(e.rule)} · ${esc(e.code)} <span>${esc(e.reason)}</span>${e.failed_check ? `<span>${classTag(e.failure_class)} ${esc(((state.failure_classes || {})[classId(e.failure_class)] || {}).note || '')}</span>` : ''}</p>${e.decision === 'ESCALATE' ? heldHtml(r) : ''}</div>
    ...
    function renderExceptions() {
      ...
      if ($('ex-meta')) $('ex-meta').textContent = mine.length ? `${mine.length} flags · ${mine.filter(x => x.status === 'OPEN').length} awaiting review` : 'no flags';
      ...
      if (!mine.length) tb.append(el('tr', null, '<td colspan="4" class="empty-row">Nothing flagged yet.</td>'));
  ...
  async function customer() {
    ...
    const txKey = (inst) => inst ? `${inst.invoice_ref || ''}|${Number(inst.amount || 0)}|${String(inst.payee_account_ref || '').replace(/\D/g, '')}` : '';
    let liveRows = [];
    async function refreshDecisions() {
      if (!snap) return;
      try {
        const live = await api('GET', '/api/audit'); liveRows = live.rows || [];
        const snapHeld = snap.audit.rows.filter(r => r.kind === 'verify' && r.entry.decision === 'ESCALATE');
        const mapped = (liveRows.filter(r => r.kind === 'decision')).map(dd => { const target = snapHeld.find(r => txKey(r.entry.instruction) === txKey(dd.entry.instruction)); return target ? { ...dd, entry: { ...dd.entry, held_audit_id: target.id } } : null; }).filter(Boolean);
        const ids = new Set(mapped.map(x => x.entry.held_audit_id));
        lastDecisions = [...mapped, ...snap.state.decisions.filter(x => !ids.has(x.entry.held_audit_id))]; snap.state.decisions = lastDecisions;
      } catch (e) { /* live unavailable: the snapshot stands */ }
    }
    // the live record behind a snapshot row: the held instruction with the same invoice, amount and payee
    const liveIdFor = (r) => { if (!snap) return r.id; const hit = liveRows.find(x => x.kind === 'verify' && x.entry.decision === 'ESCALATE' && txKey(x.entry.instruction) === txKey(r.entry.instruction)); return hit ? hit.id : r.id; };
    ...
    // what matters to the customer: holds for review, decisions by a person, the mandate; never a routine processed payment
    ...
    async function renderAccount() {
      ...
      const live = mine.filter(r => r.kind === 'verify').map(r => { const e = r.entry, i = e.instruction || {}; const dd = e.decision === 'ESCALATE' ? decisionFor(r.id) : null; const moved = e.decision === 'ALLOW' || (dd && dd.entry.outcome === 'RELEASED'); const word = moved ? '' : (dd ? 'Declined' : 'For review'); const byWhom = e.code === FIRST ? 'you' : 'your approver'; return { ai: { supplier: i.supplier_name || '', agent: agentOf(r.subject), invoice: i.invoice_ref, amount: Number(i.amount || 0), word, tone: e.failed_check ? (classId(e.failure_class) === 'fraud' ? 'fraud' : 'error') : '', audit: r.id }, d: r.ts, desc: `${word ? `<b class="txn-blocked">${word}</b> · ` : ''}<b class="txn-ai">AI agent</b> ${esc(agentOf(r.subject))} · ${esc(i.supplier_name || '')} · <span class="mono">${esc(i.payee_account_ref || '')}</span> · invoice ${esc(i.invoice_ref || '')}${word ? ` · <span class="small">${e.decision === 'ESCALATE' ? (dd ? 'by ' + byWhom : 'for ' + byWhom) : 'Agent Passport ' + esc(e.rule)}</span>` : (dd ? ` · <span class="small">${e.code === FIRST ? 'first payment, confirmed by you' : 'released by your approver'}</span>` : '')}`, out: moved ? Number(i.amount || 0) : null, blocked: !moved, ts: r.ts }; });
      const all = [...live, ...STATIC_TXNS.map(x => ({ ...x, ts: x.d + 'T12:00:00Z' }))].map((x, k) => ({ ...x, k })).sort((a, b) => (a.ts < b.ts ? 1 : a.ts > b.ts ? -1 : a.k - b.k));
      // anything waiting for a person sits at the top of the log
      all.sort((a, b) => ((b.ai && b.ai.word === 'For review') ? 1 : 0) - ((a.ai && a.ai.word === 'For review') ? 1 : 0));
    ...
    function renderEvent() {
      ...
      if (r.kind === 'verify') {
        ...
        const why = plainWhy();
        steps.push({ who: 'bank', t: r.ts, title: e.decision === 'ESCALATE' ? `${bank} is holding it for review` : `${bank} paid it`, body: why + (e.decision === 'ESCALATE' && e.failed_check ? ' Nothing has left your account.' : ''), proof: '' });
        if (e.decision === 'ESCALATE') {
          const first = e.code === FIRST;
          if (dd) { const who = esc(((dd.entry.decided_by || dd.entry.approver) || {}).name || 'Your approver'); const rel = dd.entry.outcome === 'RELEASED'; steps.push({ who: 'you', t: dd.ts, title: `${who} ${rel ? (first ? 'confirmed it' : 'approved it') : 'declined it'}`, body: `${rel ? `Paid ${amt} to ${sup}.${first ? ' Payments inside the mandate now go through without confirmation.' : ''}` : 'Nothing left your account.'}${dd.entry.reason && !first ? ` <q class="tl__note">${esc(dd.entry.reason)}</q>` : ''}`, proof: '' }); }
          else if (first) steps.push({ who: 'you', t: null, title: 'Waiting for you', body: 'Review it and confirm or decline. Nothing moves until you do.', act: `<button class="btn" type="button" data-review="${r.id}">${pendingConfirm === r.id ? 'Hide the review' : 'Review and confirm'}</button>${pendingConfirm === r.id ? confirmHtml(n) : ''}`, proof: '' });
          else steps.push({ who: 'you', t: null, title: 'Your decision', body: 'Write a short note on why, then approve it or decline it. Both are recorded with your name.', act: `<div class="cu-review"><label class="label" for="cu-review-note">Your note</label><textarea class="textarea" id="cu-review-note" rows="2" placeholder="${e.failed_check ? 'e.g. Ashby quoted the larger order in writing; approved this once.' : 'e.g. Checked the invoice with the supplier; approve.'}"></textarea><p class="error" id="cu-review-err" hidden>Write a note first; it is recorded with your decision.</p><div class="actions"><button class="btn" type="button" data-decide-cu="release" data-audit="${r.id}">Approve and pay</button><button class="btn btn--secondary" type="button" data-decide-cu="refuse" data-audit="${r.id}">Decline</button><span class="actions__note">Recorded as ${esc(ap.name || '')}, ${esc(ap.role || '')}.</span></div></div>`, proof: '' });
    ...
    function noteFor(r) {
      ...
      if (r.kind === 'verify' && e.decision === 'ESCALATE' && e.failed_check) {
        ...
        if (classId(e.failure_class) === 'fraud') {
          const text = e.code === 'PAYEE_NOT_ON_MANDATE' ? `${ag} tried to pay ${sup} ${amt} to an account not on your mandate. The bank is holding it for your review. Nothing has left your account.`
            : e.code === 'REPLAY_DETECTED' ? `A payment ${ag} had already made, ${sup} ${amt}, was presented again. The bank is holding the repeat for your review.`
            : e.code === 'AGENT_SIGNATURE_INVALID' ? `Something presented the passport of ${ag} without its key, to pay ${sup} ${amt}. The bank is holding it for your review.`
            : `An instruction for ${sup} ${amt} on the passport of ${ag} did not pass the bank's checks. The bank is holding it for your review.`;
          return { ...base, tone: 'red', label: 'For your review · fraud indicator', review: true, text, sub: 'Nothing moves until you approve or decline it. The bank\'s risk team has the record.' };
        }
        const how = e.code === 'PER_PAYMENT_LIMIT_EXCEEDED' ? `more than the ${gbp((((passOf(r.subject) || {}).mandate || {}).authorization_details || [{}])[0].per_payment_limit ? (((passOf(r.subject) || {}).mandate || {}).authorization_details || [{}])[0].per_payment_limit.amount : 0)} a payment its mandate allows` : e.code === 'MONTHLY_LIMIT_EXCEEDED' ? 'more than its mandate allows this supplier in 30 days' : e.code === 'CURRENCY_NOT_PERMITTED' ? 'in the wrong currency' : e.code === 'OUT_OF_SCOPE' ? 'for an action it is not permitted' : e.code === 'DAILY_COUNT_EXCEEDED' ? 'past its daily count' : ['MANDATE_EXPIRED', 'MANDATE_REVOKED', 'MANDATE_NOT_SIGNED'].includes(e.code) ? 'without a mandate in force' : 'outside its limits';
        return { ...base, tone: 'amber', label: 'For your review · over the mandate', review: true, text: `${ag} asked to pay ${sup} ${amt}, ${how}. The bank is holding it for your review.`, sub: 'Approve it with a note and it is paid; decline it and nothing moves.' };
      }
      if (r.kind === 'verify' && e.decision === 'ESCALATE') {
        ...
        return { ...base, tone: 'amber', label: 'For your review', review: true, text: `A payment above your approval threshold is waiting for ${esc(ap.name || 'your approver')}.`, sub: `${sup}, ${amt}, held at the bank. Nothing moves until ${esc(ap.name || 'your approver')} approves or declines it.` };
      }
      if (r.kind === 'decision') {
        const name = esc(((e.decided_by || e.approver) || {}).name || 'your approver'); const released = e.outcome === 'RELEASED';
        if (e.first_payment) return { ...base, tone: 'slate', label: released ? 'First payment confirmed' : 'First payment declined', text: released ? `${name} confirmed the first payment under mandate version ${esc(String(e.mandate_version || 1))}: ${sup} ${amt} was paid. Payments inside the mandate now flow without confirmation.` : `${name} declined the first payment under this mandate (${sup} ${amt}). Nothing left your account; the next attempt will ask again.`, sub: 'Recorded in the evidence chain with a receipt signed by the bank.' };
        return { ...base, tone: 'slate', label: released ? 'Approved after review' : 'Declined after review', text: released ? `${name} approved the payment of ${amt} to ${sup} after review.` : `${name} declined the payment of ${amt} to ${sup} after review. Nothing left your account.`, sub: e.reason ? `“${esc(e.reason)}”` : `Recorded as ${esc(e.officer || '')} in the evidence chain.` };
      }
      if (r.kind === 'mandate') {
      ...
      if (r.kind === 'incident') return { ...base, tone: 'red', label: 'Incident', text: `The bank raised an incident on ${ag} after ${esc(String(e.denies))} instructions held for review.`, sub: 'Its payments risk team is looking. Nothing left your account.' };
    ...
    $('cu-event-body').addEventListener('click', async (ev) => {
      const dc = ev.target.closest('[data-decide-cu]'); if (dc && !dc.disabled) { const note = ($('cu-review-note') || { value: '' }).value.trim(); if (!note) { $('cu-review-err').hidden = false; return; } dc.disabled = true; try { const target = acRows.find(x => x.id === +dc.dataset.audit); await refreshDecisions(); await api('POST', `/api/audit/${target ? liveIdFor(target) : dc.dataset.audit}/decide`, { decision: dc.dataset.decideCu, reason: note, by: 'customer' }); } catch (e) { alert(e.message); dc.disabled = false; return; } try { sessionStorage.setItem('ap-toast', dc.dataset.decideCu === 'release' ? '<b>Approved.</b> The payment is on its way. Your note is on the record.' : '<b>Declined.</b> Nothing left your account. Your note is on the record.'); } catch (e) { /* no storage */ } location.href = '/customer'; return; }
    ...
    function renderPassport(p, show) {
      ...
      const signed = (who, when) => `<span class="pp__signed">Signed by ${esc(who)}${when ? ` · ${d(when)}` : ''}</span>`;
      const row = (k, val) => val != null && val !== '' ? `<div><dt>${k}</dt><dd>${val}</dd></div>` : '';
      box.innerHTML = `<div class="pp__book">
        <div class="pp__cover">
          <span class="pp__wordmark">Agent Passport</span>
          <div class="pp__holder"><b>${esc(ag.name || '')}</b><span>${esc(pr.product_name || '')} by ${esc(pr.provider || '')}</span></div>
          <dl class="pp__ids">${row('Number', `<span class="mono">${esc(p.passport_id)}</span>`)}${row('Status', tag(p.status))}${row('Issued', d(p.issued_at))}${row('Valid to', d(a.valid_until))}${row('First payment', p.first_payment_confirmed ? 'confirmed by you' : 'awaits your confirmation')}</dl>
          <span class="pp__issuer">Issued by ${bankName}</span>
        </div>
        <div class="pp__pages">
          <section class="pp__page"><h3 class="pp__h">Admission</h3>${signed(bankName, a.iat ? new Date(a.iat * 1000).toISOString() : p.issued_at)}<dl class="pp__kv">${row('Assurance', (LEVELS[(a.assurance_evidence || {}).level] || ['not declared'])[0])}${row('Bank holds above', gbp(((a.condition || {}).hold_above || {}).amount))}</dl></section>
          <section class="pp__page"><h3 class="pp__h">Agent identity</h3>${signed('you', p.issued_at)}<dl class="pp__kv">${row('Model', `${esc(ag.model_provider || '')} <span class="mono">${esc(ag.model_version || '')}</span>`)}${row('Key', `<span class="mono">${esc(key.kid || '')}</span>${key.pop_verified ? ' · possession proven' : ''}`)}</dl></section>
          <section class="pp__page"><h3 class="pp__h">Mandate <span>version ${esc(String(m.version || 1))}</span></h3>${signed((m.signed_by || {}).name || 'you', p.mandate_signed_at)}<dl class="pp__kv">${row('Per payment', gbp((ad.per_payment_limit || {}).amount))}${row('Per supplier, 30 days', gbp((ad.monthly_limit_per_account || {}).amount))}${row('Per day', `${esc(String(ad.max_payments_per_day || ''))} payments`)}${row('Expires', d(m.valid_until))}${row('May pay only', `<span class="pp__payees">${payees}</span>`)}</dl></section>
        </div>
      </div>`;
    }
    function render() {
      ...
      const pl = $('cu-products'); if (pl) { pl.innerHTML = products.map(m => { const c = ((m.ceilings || {}).per_payment_ceiling || {}).amount; return `<button type="button" class="cu-product" role="radio" data-id="${m.registration_id}" aria-checked="${String(m.registration_id) === sel.value}"><span class="cu-product__avatar" aria-hidden="true">${esc(String(m.product_name || '').split(/[\s·]+/).filter(w => /^[A-Za-z]/.test(w)).slice(0, 2).map(w => w[0].toUpperCase()).join('') || '·')}</span><span class="cu-product__body"><b>${esc(m.product_name)}</b><span class="small">${esc(m.provider)} · ${esc((LEVELS[m.assurance_level] || ['no level'])[0].toLowerCase())}</span></span><span class="cu-product__mark" aria-hidden="true"></span></button>`; }).join('') || '<p class="small">Your bank has not admitted any AI product yet.</p>'; pl.onclick = (ev) => { const c = ev.target.closest('.cu-product'); if (!c) return; sel.value = c.dataset.id; showLevel(); }; }
      showLevel();
    ...
    function renderForm() {
      ...
      (draft.supplier_allowlist || []).forEach((s, i) => {
        const onFile = !!s.account_ref && !s.editing;
        ul.append(el('li', 'cu-payee', `<span class="cu-product__avatar" aria-hidden="true">${esc(ini(s.name))}</span><div class="cu-payee__body"><b>${esc(s.name)}</b><small id="cu-ch-${i}">${s.companies_house_number ? `Companies House ${esc(s.companies_house_number)}` : 'not on the register'}</small>${onFile ? `<small class="cu-payee__acct">Account on file <span class="mono">${esc(s.account_ref)}</span></small><input type="hidden" data-i="${i}" data-k="account_ref" value="${esc(s.account_ref)}">` : s.editing ? `<input class="input mono" data-i="${i}" data-k="account_ref" value="${esc(s.account_ref || '')}" aria-label="Sort code and account number for ${esc(s.name)}" placeholder="Sort code and account, 60-11-22 12345678">` : `<input type="hidden" data-i="${i}" data-k="account_ref" value=""><button class="link cu-payee__add" type="button" data-edit="${i}">Add bank account</button>`}<input type="hidden" data-i="${i}" data-k="name" value="${esc(s.name)}"><input type="hidden" data-i="${i}" data-k="companies_house_number" value="${esc(s.companies_house_number || '')}"></div><button class="link cu-payee__remove" type="button" data-remove="${i}" aria-label="Remove ${esc(s.name)}">Remove</button>`));
    ...
    const onFileFor = (number, name) => { const known = (d0.supplier_allowlist || []); const hit = known.find(x => x.companies_house_number && x.companies_house_number === number) || known.find(x => String(x.name || '').toLowerCase() === String(name || '').toLowerCase()); return hit ? hit.account_ref : ''; };
    $('cu-payee-results').addEventListener('click', (e) => { const b = e.target.closest('[data-add]'); if (!b) return; collect(); const acct = onFileFor(b.dataset.add, b.dataset.name); draft.supplier_allowlist.push({ supplier_id: `SUP-${String(draft.supplier_allowlist.length + 1).padStart(3, '0')}`, name: b.dataset.name, account_ref: acct, companies_house_number: b.dataset.add, editing: !acct }); $('cu-payee-search').value = ''; $('cu-payee-results').hidden = true; renderForm(); const acc = document.querySelector(`#cu-payees [data-i="${draft.supplier_allowlist.length - 1}"][data-k="account_ref"]:not([type="hidden"])`); if (acc) acc.focus(); scheduleCheck(); });
    $('cu-payees').addEventListener('click', (e) => { const b = e.target.closest('[data-edit]'); if (!b) return; collect(); draft.supplier_allowlist[+b.dataset.edit].editing = true; renderForm(); const acc = document.querySelector(`#cu-payees [data-i="${b.dataset.edit}"][data-k="account_ref"]:not([type="hidden"])`); if (acc) acc.focus(); });
  ...
  async function console_() {
    ...
    async function judge(b, r, k, fast) {
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        if (stopped || !tr) { setCell(li, 'skip', 'not evaluated · held by default'); await pause(step / 4); continue; }
        $('g-count').textContent = `${i + 1} / ${ids.length}`;
        setCell(li, 'checking', 'checking…'); await pause(step);
        setCell(li, tr.ok ? 'pass' : (r.failed_check ? 'fail' : 'hold'), esc(tr.note));
      ...
      const held = r.decision === 'ESCALATE' ? (r.code === FIRST ? `<span class="hold">held for the customer: the first payment under this mandate version is confirmed once, in its banking app <a href="/customer?ref=${esc(r.passport_id || (p || {}).passport_id || '')}">→ Customer dashboard</a></span>` : r.failed_check ? `<span class="hold">held for review: nothing is refused outright; a person approves or declines it with a note <a href="/bank">→ Bank dashboard</a></span>` : `<span class="hold">held for the customer's named approver; the bank's officer records the decision <a href="/bank">→ Bank dashboard</a></span>`) : '';
      const inc = r.incident ? `<span class="fail"><b>INCIDENT</b> raised to the payments risk team · ${r.incident.denies} instructions held for review <a href="/bank">→ Bank console</a></span>` : '';
    ...
    function logLine(b, r, k) {
      ...
      if (r.incident) { lines.append(el('li', 't-incident', `<span><b>INCIDENT</b> raised to the payments risk team · ${r.incident.denies} instructions held for review · evidence #${r.incident.audit_id}</span>`)); setDenies(0); }
      else if (r.failed_check) setDenies(Math.min(3, (r.deny_count || denies + 1)));
    }
    ...
    async function showInvoice(r) {
      ...
      $('inv-caption').textContent = ok
        ...
        : 'The AI agent read a manipulated invoice and would have paid the wrong account. The mandate held it for a person.';
      $('inv-caption').hidden = false;
      const con = $('inv-contrast'); con.hidden = ok;
      if (!ok) con.innerHTML = '<b>Today:</b> a disputed payment, no record of what the AI agent was permitted to do. <b>Here:</b> an instruction held for review, a flagged entry for the bank\'s risk team, and a replayable record showing the document changed the destination, not the agent.';
    }
    ...
    function termLine(b, r, k) {
      ...
      return el('li', null, `<span class="t-time">${t(new Date().toISOString())}</span><span class="t-body">
        ...
        <span class="t-reason">${esc(r.reason)}${r.failed_check && r.rule === 'R.6' ? ' · Named-beneficiary mandate check (FATF 2025 AML/CFT alignment)' : ''}</span>${sigLine}${chainLine}${settle}${rails}
