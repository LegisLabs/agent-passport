...
const AP = (() => {
  ...
  async function customer() {
    ...
    function renderEvent() {
      ...
      const proof = (row, extra) => row ? `<span class="tl__proof">record #${row.id}${row.receipt ? ' · receipt signed by the bank' : ''}${extra || ''}</span>` : '';
      const steps = [];
      if (r.kind === 'verify') {
        ...
        const why = e.decision === 'ALLOW' ? 'All nine passed.' : e.code === 'PAYEE_NOT_ON_MANDATE' ? `Stopped at check ${passed + 1}: that account is not one you approved for ${sup}.` : e.code === FIRST ? `Passed the first eight. Check ${passed + 1} holds the first payment under a new mandate for you.` : e.code === 'HUMAN_CONFIRMATION_REQUIRED' ? `Passed the first eight. Check ${passed + 1} holds anything above ${gbp(hold)} for ${esc(ap.name || 'your approver')}.` : e.code === 'PER_PAYMENT_LIMIT_EXCEEDED' ? `Stopped at check ${passed + 1}: more than the ${gbp((ad.per_payment_limit || {}).amount)} a payment you allow.` : e.code === 'MONTHLY_LIMIT_EXCEEDED' ? `Stopped at check ${passed + 1}: past the ${gbp((ad.monthly_limit_per_account || {}).amount)} you allow ${sup} in 30 days.` : e.code === 'CURRENCY_NOT_PERMITTED' ? `Stopped at check ${passed + 1}: not in pounds.` : e.code === 'REPLAY_DETECTED' ? `Stopped at check ${passed + 1}: this exact payment had already been made.` : e.code === 'AGENT_SIGNATURE_INVALID' ? `Stopped at check ${passed + 1}: not signed by your agent's key.` : `Stopped at check ${passed + 1}: ${esc(trace[stopAt] ? trace[stopAt].note : e.reason)}.`;
        steps.push({ who: 'bank', t: r.ts, title: `${bank} checked it against your mandate`, body: why, strip: Array.from({ length: 9 }, (_, k) => { const st = trace[k]; return st ? `<i class="${st.ok ? 'ok' : (e.decision === 'ESCALATE' ? 'hold' : 'fail')}" title="${esc(st.rule)} ${esc(st.title)}">${k + 1}</i>` : `<i class="skip" title="not reached">${k + 1}</i>`; }).join(''), proof: '' });
        if (e.decision === 'DENY') { const fraud = classId(e.failure_class) === 'fraud'; steps.push({ who: 'bank', t: r.ts, title: 'Refused and written down', body: fraud ? `Recorded as a fraud indicator for the bank's risk team${inc ? `; an incident was raised` : ''}. A supervisor can ask for this record.` : 'Recorded as an agent error: a quality signal about your agent, not a report to anyone.', proof: proof(r) }); steps.push({ who: 'you', t: r.ts, title: 'Nothing left your account', body: fraud ? 'Nothing for you to do.' : 'If this keeps happening, raise it with your provider.', proof: '' }); }
        ...
        } else steps.push({ who: 'bank', t: r.ts, title: 'Paid', body: `${amt} left your account for ${sup}.`, proof: proof(r) });
