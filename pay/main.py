...
Layers: the provider files an AI product on the register (/provider) → the bank admits it to its list (/bank)
        ...
        → the evidence trail (on the bank dashboard) that a supervisor can request through normal supervisory processes (/api/evidence).
"""
...
for _name in ("provider", "customer"):
    def _make(name):
