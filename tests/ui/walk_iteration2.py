...
def issue(page, sign=True):
    """Phase 1 → 2 → 3 through the UI: register model, approve, create agent, attest, sign mandate."""
    ...
    page.click("#btn-submit"); page.wait_for_selector("#confirm:not([hidden])")
    ...
    page.goto(BASE + "/customer?new=1"); page.wait_for_selector("#btn-create-agent:not([disabled])"); page.click("#btn-create-agent")
    page.wait_for_selector("#btn-ag-sign:not([hidden])"); page.click("#btn-ag-sign"); page.wait_for_selector("#btn-attest:not([hidden])"); page.click("#btn-attest")
    page.wait_for_selector("#cu-mandate:not([hidden])")
...
def main():
  with sync_playwright() as pw:
      ...
      pid = issue(page)
      page.goto(BASE + "/regulator"); page.wait_for_selector("#review > li"); page.wait_for_selector("#envelope section")
      check(page.locator("#review > li").count() == 6, "six steps rendered")
      check("model approval" in low(page, "#review") or "approve the model" in low(page, "#review"), "sign-off step approves the model")
      ...
      check("model approval" in low(page, "#rg-model") and "suspend model approval" in low(page, "#rg-model"), "model registry panel with model-level lifecycle")
