...
def issue(page, sign=True):
    ...
    ref = page.locator("#confirm-ref").inner_text()
    page.goto(BASE + f"/bank?ref={ref}"); page.wait_for_selector("#rg-case:not([hidden])")
...
def invoice(page, which):
    page.goto(BASE + "/terminal?console=1"); page.wait_for_selector("#invoice-buttons button")
...
def main():
  with sync_playwright() as pw:
      ...
      pid = issue(page)
      ref = page.request.get(BASE + "/api/state").json()["applications"][-1]["ref"]
      page.goto(BASE + f"/bank?ref={ref}"); page.wait_for_selector("#review > li")
      ...
      page.goto(BASE + f"/bank?passport={pid}"); page.wait_for_selector("#envelope section")
      ...
      page.goto(BASE + "/bank"); page.wait_for_selector("#rg-violations tbody tr")
      check(page.locator("#rg-alert").is_hidden(), "no alert after one refusal")
      invoice(page, 1)
      page.goto(BASE + "/bank"); page.wait_for_selector("#rg-alert:not([hidden])")
      ...
      page.goto(BASE + f"/bank?passport={pid}"); page.wait_for_selector("#ex-actions button")
      ...
      page.goto(BASE + "/terminal?console=1"); page.wait_for_selector("#beats button"); page.locator("#beats button").nth(7).click(); page.wait_for_selector("#term-lines li .t-verdict", state="attached")
      check("r.2" in low(page, "#term-lines li:has(.t-verdict)"), "suspended: probe DENY R.2")
      page.goto(BASE + f"/bank?passport={pid}"); page.wait_for_selector("#ex-actions button"); page.fill("#rg-reason", "misuse confirmed")
      ...
      pid2 = issue(page)
      invoice(page, 1)
      page.goto(BASE + f"/bank?passport={pid2}"); page.wait_for_selector("#ex-actions button"); page.fill("#rg-reason", "looks odd")
      page.click("#ex-actions button:has-text('Suspend')"); page.wait_for_function("document.querySelector('#ex-state').innerText.toLowerCase().includes('suspended')")
